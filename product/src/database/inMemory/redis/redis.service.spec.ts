...
describe("RedisService", () => {
    ...
    beforeEach(async () => {
        ...
        testKeyValueMap.forEach((value, key) => {
            msetCommand.push(TEST_KEY_PREFIX+key, value);
        });
    ...
    describe('getAllKeys', () => {
        it("모든 키를 배열로 반환.", async () => {
            ...
            allKeys.forEach((key) => {
                if (key.slice(0, TEST_KEY_PREFIX.length) === TEST_KEY_PREFIX) testKeyValueMap.delete(key);
            });
